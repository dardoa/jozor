-- Migration: RLS Hardening and RPC Security Update
-- Description: Ensures RPCs use JWT-based identity rather than client-provided IDs.
--              Hardens tree_operations to prevent impersonation.

-- 1. Update Create Tree with Root RPC
...
BEGIN
  ...
  -- Security: Caller must match the owner_id they are trying to create a tree for
...
-- 2. Update Delete Person RPC
CREATE OR REPLACE FUNCTION delete_person_and_relations(
  p_tree_id UUID,
  p_owner_id TEXT, -- Keep for compatibility but prioritize JWT check
  p_person_id UUID
) RETURNS VOID AS $$
DECLARE
  v_caller_id TEXT;
BEGIN
  ...
  -- Verify Ownership or Editor Access using JWT
  IF NOT (
    EXISTS (SELECT 1 FROM trees WHERE id = p_tree_id AND owner_id = v_caller_id)
...
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
-- 3. Update Create Person and Relationship RPC
CREATE OR REPLACE FUNCTION create_person_and_relationship(
  p_tree_id UUID,
  p_owner_id TEXT, -- Keep for compatibility
  p_person_data JSONB,
...
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
-- 4. Harden tree_operations Policies
