...
-- Checkpoints must preserve recorded genealogy. A spouse is not implicitly a parent.
-- Existing checkpoints are not rewritten; readers reconcile their edges on load.
...
BEGIN
  ...
  person_relations AS (
    SELECT src AS person_id, rel_type, jsonb_agg(dst ORDER BY dst) AS dst_list
    FROM explicit_relations
