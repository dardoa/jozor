...
--              Uses the validated fallback view design with explicit access checks.
...
-- 4. Create public.people_secure view (fallback design running as definer/postgres)
...
-- Fix for to_jsonb(v_op->>'created_at') NULL trap in sync_tree_batch
    ...
    IF v_type = 'ADD_NODE' THEN
      ...
      FOR v_key, v_val IN SELECT * FROM jsonb_each(v_person) LOOP
        IF v_key NOT IN ('id', 'parents', 'spouses', 'children', 'partnerDetails', 'metadata') THEN
          v_init_last_updated := jsonb_set(v_init_last_updated, ARRAY[v_key], to_jsonb(v_incoming_ts::text));
    ...
    ELSIF v_type = 'UPDATE_PROP' THEN
      ...
      FOR v_key, v_val IN SELECT * FROM jsonb_each(v_updates) LOOP
        ...
        IF v_overwrite THEN
          ...
          v_metadata := jsonb_set(v_metadata, ARRAY['lastUpdated', v_key], to_jsonb(v_incoming_ts::text));
      ...
      IF v_updates IS NOT NULL THEN
        ...
        IF jsonb_exists(v_updates, 'name') AND v_updates->>'name' IS NOT NULL THEN
          ...
          IF v_overwrite THEN
            ...
            v_tree_settings := jsonb_set(v_tree_settings, '{sync_metadata,lastUpdated,name}', to_jsonb(v_incoming_ts::text));
        ...
        IF jsonb_exists(v_updates, 'focusId') AND v_updates->>'focusId' IS NOT NULL THEN
          ...
          IF v_overwrite THEN
            ...
            v_tree_settings := jsonb_set(v_tree_settings, '{sync_metadata,lastUpdated,focusId}', to_jsonb(v_incoming_ts::text));
        ...
        IF jsonb_exists(v_updates, 'settings') AND jsonb_typeof(v_updates->'settings') = 'object' THEN
          FOR v_key, v_val IN SELECT * FROM jsonb_each(v_updates->'settings') LOOP
            ...
            IF v_overwrite THEN
              ...
              v_tree_settings := jsonb_set(v_tree_settings, ARRAY['sync_metadata', 'lastUpdated', v_key], to_jsonb(v_incoming_ts::text));
