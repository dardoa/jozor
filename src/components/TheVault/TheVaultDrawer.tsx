...
import { useVaultTreeManagement } from './useVaultTreeManagement';
...
import type { MobileManagementSection, MobileVaultHub, VaultRenderContext, VaultStats, VaultTab } from './vaultDrawerTypes';
...
export const TheVaultDrawer: React.FC<TheVaultDrawerProps> = ({
  ...
  }, [clearTrees, currentUser, googleSync, isVaultOpen, loadTrees]);
  ...
  return (
    <>
      ...
      <OverlayPrimitive isOpen={isVaultOpen} onClose={() => setVaultOpen(false)} id="the-vault-drawer" withBackdrop={false}>
        ...
        />
        <div className={`fixed z-[calc(var(--z-index-drawer)+1)] flex flex-col overflow-hidden border border-[var(--border-main)] bg-[var(--theme-bg)] shadow-[0_30px_90px_rgba(44,24,16,0.18)] ${isMobile ? 'inset-0 h-[100dvh] w-full rounded-none border-0' : 'inset-x-4 top-[5vh] bottom-[5vh] rounded-[24px] sm:start-1/2 sm:w-[960px] sm:-translate-x-1/2'}`}>
          <div className={`ds-sidebar-header flex shrink-0 items-center justify-between border-b border-[var(--border-soft)] ${isMobile ? 'px-4 py-4' : 'px-6 py-4'}`}>
