...
import { useSmartPersonaDrawerState } from './useSmartPersonaDrawerState';
    ...
    return (
      <>
        ...
        <aside
          ...
          className={`ds-sidebar-shell fixed inset-x-0 bottom-0 sm:bottom-auto sm:top-14 md:top-16 z-[calc(var(--z-index-drawer)+1)] flex w-full max-w-full min-h-0 flex-col border-t sm:border-t-0 transition-all duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] rounded-t-[24px] sm:rounded-none sm:end-0 sm:start-auto sm:border-inline-start-0 ltr:sm:start-0 ltr:sm:end-auto ltr:sm:border-r rtl:sm:right-0 rtl:sm:left-auto rtl:sm:border-l ${isMobileViewport ? 'inset-0 rounded-none border-0' : ''} ${sizeClasses}`}
          style={{
        ...
        >
          ...
          <div className="no-drag ds-sidebar-header shrink-0">
            {smartPersonaSize !== 'collapsed' && person && (
              <div className="border-b border-[var(--border-soft)] px-4 pb-4 pt-2 sm:hidden">
                <div className="flex items-start justify-between gap-3 rounded-[24px] bg-[var(--surface-panel)] px-4 py-4 shadow-[var(--shadow-sm)]">
                  <div className="flex min-w-0 items-center gap-3">
                    <SmartAvatar person={person} size={64} className="rounded-2xl" />
                    <div className="min-w-0">
                      <h2 className="truncate text-lg font-bold text-[var(--text-main)]">{safeDisplayName}</h2>
                      <p className="mt-1 text-sm text-[var(--text-muted)]">{safeBirthYear || 'No birth date recorded'}</p>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {person.spouses.length > 0 && (
                          <span className="rounded-full border border-[var(--border-soft)] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[var(--text-dim)]">
                            {person.spouses.length} relationships
                          </span>
                        )}
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="min-h-12 min-w-12 shrink-0 rounded-full border border-[var(--border-soft)] bg-[var(--surface-app)] text-[var(--text-main)] shadow-[var(--shadow-sm)]"
