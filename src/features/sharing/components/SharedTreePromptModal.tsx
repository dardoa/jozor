...
export const SharedTreePromptModal: React.FC<SharedTreePromptModalProps> = ({
    ...
    return (
        <OverlayPrimitive
            ...
            backdropClassName="fixed inset-0 z-[var(--z-index-modal)] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300"
        >
            <div 
                className="relative z-[calc(var(--z-index-modal)+1)] w-full max-w-md bg-stone-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300"
                    ...
                    <div className="space-y-3">
                        {sharedTrees.map((tree) => (
                            ...
                            >
                                ...
                                <ArrowRight className="w-4 h-4 text-gray-500 group-hover:text-white group-hover:translate-x-1 rtl:group-hover:-translate-x-1 transition-all rtl:rotate-180" />
                            </button>
                        ))}
                    </div>
                </div>
