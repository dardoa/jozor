...
import { SharedTreeLoader } from '../components/SharedTreeLoader';
...
describe('SharedTreeLoader', () => {
  ...
  it('shows the access denied state when accepting the invite token fails', async () => {
    ...
    expect(screen.getByText('Some data is incomplete or invalid. Please review the latest change.')).toBeInTheDocument();
  });
  ...
  it('shows the access denied state when a database tree resolves without collaborator access', async () => {
    ...
    expect(onLoadComplete).not.toHaveBeenCalled();
    expect(screen.getByText('You do not have permission to make this change.')).toBeInTheDocument();
  ...
  it('blocks non-database shared-tree links before loading proxy data', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to access the shared file.')).toBeInTheDocument();
    });
