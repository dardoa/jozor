...
describe('OfflineCache', () => {
    it('saves a single pending operation and removes localId/id during sanitization', async () => {
        const op: PendingDeltaOp = { localId: 12, id: 'operation-34', tree_id: 'tree-1', user_id: 'user-1', type: 'ADD_NODE', payload: {}, created_at: '2026' };
        vi.mocked(storageService.savePendingOperation).mockResolvedValueOnce(42);

        const result = await offlineCache.savePendingOperation(op);
    ...
    it('saves multiple pending operations in bulk and sanitizes them', async () => {
        const ops: PendingDeltaOp[] = [
            { localId: 10, id: 'operation-11', tree_id: 'tree-1', user_id: 'user-1', type: 'ADD_NODE', payload: {}, created_at: '2026' },
            { localId: 20, id: 'operation-22', tree_id: 'tree-1', user_id: 'user-1', type: 'UPDATE_PROP', payload: {}, created_at: '2026' }
        ];
        vi.mocked(storageService.savePendingOperations).mockResolvedValueOnce([42, 43]);

        const result = await offlineCache.savePendingOperations(ops);
