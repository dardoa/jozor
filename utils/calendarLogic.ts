    ...
    Object.values(people).forEach(person => {
        ...
        if (person.birthDate && !person.isDeceased) {
            ...
            if (bday) {
                ...
                events.push(`DESCRIPTION:Born in ${getDisplayDate(person.birthDate)}` + (person.birthPlace ? ` at ${person.birthPlace}` : ''));
        ...
        if (person.isDeceased && person.deathDate) {
            ...
            if (dday) {
                ...
                events.push(`DESCRIPTION:Passed away in ${getDisplayDate(person.deathDate)}` + (person.deathPlace ? ` at ${person.deathPlace}` : ''));
