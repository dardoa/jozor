...
const findAndValidateMediaFile = async (
    ...
    if (mediaPath.startsWith('/') || mediaPath.includes('../') || mediaPath.length > 255) {
        console.warn(`Security alert: Invalid or suspicious media path detected: '${mediaPath}'. Skipping.`);
    ...
    if (!mediaPath.startsWith(`${expectedFolder}/`)) {
        console.warn(`Security alert: Media path '${mediaPath}' does not start with expected folder '${expectedFolder}'. Skipping.`);
        return mediaPath;
    }
    ...
    if (mediaFile) {
        ...
        const mime = ext === 'png' ? 'image/png' : ext === 'jpg' ? 'image/jpeg' : ext === 'jpeg' ? 'image/jpeg' : ext === 'webp' ? 'image/webp' : 'audio/webm';
