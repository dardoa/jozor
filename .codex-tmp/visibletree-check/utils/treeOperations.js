...
const removeId = (arr, idToRemove) => arr.filter((id) => id !== idToRemove);
/**
 ...
 */
export const performDeletePerson = (people, idToDelete) => {
...
export const performAddParent = (people, currentId, gender, relatedPersonId) => {
    const current = people[currentId];
    if (!current || current.parents.length >= 2)
        return null;
 ...
 */
export const performAddSpouse = (people, currentId, gender) => {
    const current = people[currentId];
    if (!current)
        return null;
 ...
 */
export const performLinkPerson = (people, currentId, targetId, type, relatedPersonId) => {
    const nextPeople = { ...people };
    ...
    if (!current || !target)
        return people;
    if (type === 'parent') {
        if (current.parents.includes(targetId) || current.parents.length >= 2)
            return people;
        ...
        const coParentId = relatedPersonId || (current.spouses.length === 1 ? current.spouses[0] : undefined);
        if (coParentId) {
 ...
 */
export const performRemoveRelationship = (people, id1, id2, type) => {
    const nextPeople = { ...people };
    ...
    if (type === 'parent') {
        ...
        // Clean details
        if (nextPeople[id1].partnerDetails)
            delete nextPeople[id1].partnerDetails[id2];
        if (nextPeople[id2].partnerDetails)
            delete nextPeople[id2].partnerDetails[id1];
    }
    return nextPeople;
};
