 ...
 */
export const createPerson = (gender = 'male') => ({
 ...
 */
export const getYears = (p, _format = 'iso') => {
 ...
 */
const getBirthYearForSort = (person) => {
    if (!person.birthDate)
        return Infinity;
 ...
 */
export const sortPeopleByBirthDate = (peopleList) => {
 ...
 */
export const getAncestors = (personId, people, visited = new Set()) => {
 ...
 */
export const formatDate = (dateStr, format = 'iso') => {
    if (!dateStr)
        return '';
    ...
    if (!isValid)
        return dateStr; // Return as-is if not valid date
