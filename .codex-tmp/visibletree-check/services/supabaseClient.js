 ...
 */
const createAuthInterceptorFetch = (token, _uid, _email) => {
    return (url, options) => {
 ...
 */
const createIsolatedInstance = (token) => {
    return createClient(supabaseUrl, supabaseKey, {
        ...
        global: {
            fetch: createAuthInterceptorFetch(token)
        }
    });
};
/**
 ...
 */
export const getSupabase = (uid, email, token) => {
    const cacheKey = token ? token : (uid && email ? `${uid}:${email.toLowerCase()}` : 'anonymous');
    if (!clientInstances.has(cacheKey)) {
        logInfo('supabaseClient getSupabase', 'Initializing isolated client instance.', {
            operationType: 'init_supabase_client',
        ...
        if (clientInstances.size > 20) {
            ...
            if (firstKey)
                clientInstances.delete(firstKey);
        }
    }
    return clientInstances.get(cacheKey);
};
export const getSupabaseWithAuth = (uid, email, token) => getSupabase(uid, email, token);
