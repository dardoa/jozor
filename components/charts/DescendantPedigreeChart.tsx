...
export const DescendantPedigreeChart: React.FC<DescendantPedigreeChartProps> = memo(({
  ...
  // Removed LINE_CORNER_RADIUS as it's no longer needed for elbow paths
  const isVertical = settings.layoutMode === 'vertical';

  // Removed drawChildBranchPath function as it's no longer needed

  const getLinkPath = useCallback((link: TreeLink) => {
    const sourceNode = nodes.find(n => n.id === link.source);
    const targetNode = nodes.find(n => n.id === link.target); // This is the child node
    if (!sourceNode || !targetNode) return '';
    
    if (link.type === 'marriage') {
        // Keep marriage lines as straight dashed lines
        return `M ${sourceNode.x} ${sourceNode.y} L ${targetNode.x} ${targetNode.y}`;
    }
    
    if (settings.chartType === 'pedigree') {
        // Keep pedigree lines as cubic Bezier curves
    ...
    if (settings.chartType === 'descendant' && link.type === 'parent-child') {
        const childData = targetNode.data as Person;
        let startX = sourceNode.x;
        let startY = sourceNode.y + NODE_HEIGHT / 2; // Default: from parent's bottom

        // Check for a collapse point (joint)
        let correctJoint = null;
        if (childData.parents.length > 1) {
           const otherParentId = childData.parents.find(id => people[id]?.id !== (sourceNode.data as Person).id); 
           if (otherParentId) {
              correctJoint = collapsePoints.find(cp => cp.id === (sourceNode.data as Person).id && cp.spouseId === otherParentId);
           }
        }
        if (!correctJoint) correctJoint = collapsePoints.find(cp => cp.id === (sourceNode.data as Person).id && cp.spouseId === 'single');
        
        if (correctJoint) {
            startX = correctJoint.x;
            startY = correctJoint.y; // Start from the center of the collapse circle
        }

        const targetX = targetNode.x;
        const targetY = targetNode.y - NODE_HEIGHT / 2; // To child's top

        const branchOffset = settings.isCompact ? 20 : 30; // How far down the line drops from the joint/parent before turning horizontal

        if (isVertical) {
            const midPointY = startY + branchOffset; // Drop down from source
            return `M ${startX} ${startY} V ${midPointY} H ${targetX} V ${targetY}`;
        } else { // Horizontal layout
            const midPointX = startX + branchOffset; // Drop right from source
            return `M ${startX} ${startY} H ${midPointX} V ${targetY} H ${targetX}`;
        }
    }
    return `M ${sourceNode.x} ${sourceNode.y} L ${targetNode.x} ${targetNode.y}`;
  }, [nodes, settings.chartType, collapsePoints, NODE_WIDTH, NODE_HEIGHT, people, isVertical, settings.isCompact]);
