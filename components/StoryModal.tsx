...
export const StoryModal: React.FC<StoryModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className='bg-[#fdfbf7] dark:bg-stone-900 rounded-xl shadow-2xl max-w-3xl w-full max-h-[85vh] flex flex-col border-4 border-stone-200 dark:border-stone-800 overflow-hidden relative'
        ...
        <div className='flex items-center justify-between p-5 border-b border-stone-200 dark:border-stone-800 bg-[#fdfbf7] dark:bg-stone-900'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-xs text-stone-500 font-sans flex items-center gap-1'>
                <Sparkles className='w-3 h-3' /> {t.story.aiSubtitle}
              </p>
            </div>
          </div>
          <div className='flex items-center gap-2'>
            <button
              ...
              title={t.story.regenerate}
            >
              <RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} />
            </button>
            <button
              onClick={onClose}
              className='p-2 hover:bg-stone-100 dark:hover:bg-stone-800 rounded-full transition-colors text-stone-400 hover:text-stone-600'
            >
              <X className='w-6 h-6' />
            </button>
          </div>
