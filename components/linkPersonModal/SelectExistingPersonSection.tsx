...
export const SelectExistingPersonSection: React.FC<SelectExistingPersonSectionProps> = memo(
  ({ people, type, gender, currentPersonId, familyActions, onClose }) => {
          ...
          {candidates.length === 0 ? (
            ...
            candidates.map((p) => (
              <button
                key={p.id}
                onClick={() => {
                  familyActions.onLinkPerson(p.id, type);
                  onClose();
                }}
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-sm ${p.gender === 'male' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300' : 'bg-pink-100 text-pink-600 dark:bg-pink-900 dark:text-pink-300'}`}
                >
                  {p.firstName[0]}
                </div>
                <div>
                  <div className='text-sm font-semibold text-stone-700 dark:text-stone-200 group-hover:text-blue-700 dark:group-hover:text-blue-400'>
