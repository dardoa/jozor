...
export const PartnersTab = memo<PartnersTabProps>(
  ({ person, people, isEditing, onUpdate, onSelect }) => {
    ...
    const handlePartnerUpdate = (spouseId: string, field: keyof RelationshipInfo, value: RelationshipInfo[keyof RelationshipInfo]) => {
        ...
        {!person.spouses.length && !isEditing ? (
          ...
          person.spouses.map((spouseId) => {
            ...
            return (
              <div
                key={spouseId}
                className='border border-[var(--border-soft)] rounded-xl p-2 bg-[var(--surface-subtle)] hover:border-[var(--primary-500)]/30 transition-colors shadow-[var(--shadow-sm)]'
              >
                {/* Partner Header */}
                <div className='flex items-start justify-between mb-2'>
                  <div className='flex items-center gap-2'>
                    <SmartAvatar person={spouse} size={28} className='rounded-full shadow-sm' />
                    <div>
                      ...
                      <button
                        ...
                        className='text-[11px] text-[var(--primary-600)] hover:underline flex items-center gap-1'
                      >
                ...
                {isEditing ? (
                  <div className='space-y-1.5 pt-2 border-t border-[var(--border-soft)]'>
                    ...
                    <div className='flex items-center gap-2'>
                      ...
                      <select
                        ...
                        className='flex-1 h-7 bg-[var(--surface-panel)] border border-[var(--border-soft)] rounded-lg text-xs px-2 outline-none focus:border-[var(--primary-500)] disabled:bg-transparent disabled:border-transparent disabled:px-0 disabled:font-medium text-[var(--text-main)]'
                      >
                  ...
                  // View Mode
                  <div className='space-y-1.5 pt-2 border-t border-[var(--border-soft)] text-sm text-[var(--text-main)]'>
                    <div className='flex items-center gap-2'>
