...
export const QuickAddSpeedDial: React.FC<QuickAddSpeedDialProps> = ({
  actions,
  buttonClassName = '',
}) => {
        ...
        <div className='flex gap-2 animate-in fade-in zoom-in-90 duration-200'>
          {actions.map((action, index) => (
            <button
              ...
              className={`w-6 h-6 rounded-full flex items-center justify-center transition-all shadow-sm border border-[var(--border-main)] hover:shadow-md hover:scale-110 active:scale-95 ${action.colorClasses}`}
              title={action.label}
            >
              {action.icon}
            </button>
