    ...
    return (
      <div className='bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 overflow-hidden'>
        <div className='p-4 border-b border-stone-200 dark:border-stone-700 bg-stone-50/50 dark:bg-stone-900/50 flex items-center gap-2'>
          ...
          <h4 className='text-sm font-bold text-stone-800 dark:text-stone-200'>{t.upcomingBirthdays}</h4>
        </div>
        <div className='divide-y divide-stone-50 dark:divide-stone-700'>
        ...
        <div className='flex-1 overflow-y-auto p-0 scrollbar-thin bg-black/90'>
          {activeTab === 'overview' && (
            <Suspense fallback={<StatisticsSkeleton />}>
              <StatisticsDashboard people={people} onClose={onClose} isOpen={isOpen} />
            </Suspense>
          )}
                    ...
                    diagnostics.disconnected.length > 0 ? (
                      diagnostics.disconnected.map((person) => (
                        <button
                          ...
                          className='w-full flex items-center justify-between p-3 px-4 hover:bg-stone-50 dark:hover:bg-stone-700/50 group text-start'
                        >
                          <div className='flex items-center gap-3'>
                            {person.photoUrl && (
