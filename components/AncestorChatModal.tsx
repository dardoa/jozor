...
export const AncestorChatModal = ({ isOpen, onClose, person, people }: AncestorChatModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && messages.length > 0) {
      setTimeout(() => setMessages([]), 0); // Reset on open
    }
  }, [isOpen, person.id, messages.length]);
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200'>
      <div className='bg-white dark:bg-stone-900 rounded-2xl shadow-2xl max-w-lg w-full h-[600px] flex flex-col overflow-hidden border border-stone-200 dark:border-stone-800'>
        ...
        <div
          ref={scrollRef}
          className='flex-1 overflow-y-auto p-4 space-y-4 bg-stone-50 dark:bg-stone-950/50'
        >
          {messages.length === 0 && (
