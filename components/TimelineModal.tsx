...
interface TimelineModalProps {
  ...
  language?: any; // kept but optional, as it might be passed by parent
}
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200'>
      <div className='bg-white dark:bg-stone-800 rounded-xl shadow-2xl max-w-2xl w-full flex flex-col h-[85vh] border border-stone-200 dark:border-stone-700'>
        ...
        <div className='flex flex-wrap gap-2 p-4 border-b border-stone-200 dark:border-stone-700 bg-stone-50 dark:bg-stone-900/50'>
          <span className='text-xs font-bold text-stone-500 uppercase flex items-center gap-1 me-2'>
            <Filter className='w-3.5 h-3.5' /> {t.filterBy}:
          </span>
          {eventTypes.map((type) => (
            <button
              key={type.id}
            ...
            <div className='relative border-s-2 border-stone-200 dark:border-stone-700 ms-4 space-y-8'>
              {events.map((evt, idx) => (
                <div key={idx} className='relative ps-6 group'>
                  {/* Dot */}
                  <div
                    className={`absolute -start-[9px] top-0 w-4 h-4 rounded-full border-2 border-white dark:border-stone-800 shadow-sm flex items-center justify-center 
                  ...
                  >
                    <div className='flex items-center justify-between mb-1'>
                      <span className='text-xl font-bold text-stone-300 dark:text-stone-600'>
                        {evt.year}
                      </span>
                      <div
                        className={`p-1 rounded-full ${evt.type === 'birth' ? 'bg-green-50 text-green-600' : evt.type === 'death' ? 'bg-stone-100 text-stone-500' : evt.type === 'marriage' ? 'bg-pink-50 text-pink-500' : 'bg-blue-50 text-blue-500'}`}
                      >
                    ...
                    <div className='flex items-center gap-2 mt-1'>
                      <span className='text-xs text-stone-500 font-mono bg-stone-100 dark:bg-stone-700 px-1.5 rounded'>
                        {evt.dateStr}
                      </span>
                      {evt.subLabel && (
