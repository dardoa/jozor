...
export const TimelineModal: React.FC<TimelineModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full flex flex-col h-[85vh] border border-gray-100 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50">
          <div className="flex items-center gap-2">
             ...
             <h3 className="text-lg font-bold text-gray-800 dark:text-white">{t.familyTimeline}</h3>
          </div>
          <div className="flex items-center gap-2">
            <button 
                onClick={() => setSortAsc(!sortAsc)} 
                className="text-xs font-medium text-gray-500 hover:text-blue-600 px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-md transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900 relative">
            {events.length === 0 ? (
                ...
                <div className="relative border-s-2 border-gray-200 dark:border-gray-700 ms-4 space-y-8">
                    {events.map((evt, idx) => (
                        <div key={idx} className="relative ps-6 group">
                            {/* Dot */}
                            <div className={`absolute -start-[9px] top-0 w-4 h-4 rounded-full border-2 border-white dark:border-gray-800 shadow-sm flex items-center justify-center 
                                ${evt.type === 'birth' ? 'bg-green-500' : evt.type === 'death' ? 'bg-gray-500' : 'bg-pink-500'}
                            `}>
                            </div>

                            <div 
                                onClick={() => { onSelectPerson(evt.personId); onClose(); }}
                                className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all cursor-pointer hover:border-blue-200 dark:hover:border-blue-700"
                            >
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-xl font-bold text-gray-300 dark:text-gray-600">{evt.year}</span>
                                    <div className={`p-1 rounded-full ${evt.type === 'birth' ? 'bg-green-50 text-green-600' : evt.type === 'death' ? 'bg-gray-100 text-gray-500' : 'bg-pink-50 text-pink-500'}`}>
                                ...
                                <h4 className="font-bold text-gray-800 dark:text-gray-200 text-sm">{evt.label}</h4>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="text-xs text-gray-500 font-mono bg-gray-100 dark:bg-gray-700 px-1.5 rounded">{evt.dateStr}</span>
                                    {evt.subLabel && <span className="text-xs text-gray-400 truncate max-w-[150px]">• {evt.subLabel}</span>}
                                </div>
                            </div>
                        </div>
                    ))}
